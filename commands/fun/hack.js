  ...
  run: async (client, message, args) => {
    ...
    if (!Member)
      return message.channel.send(
        `Porfavor menciona al quien quieres hackear!`
      );

    if (Member.user.id === message.author.id)
      return message.channel.send(`No te puedes hackearte at mismo lol`);
